...
class APICache: NSManagedObject {
    ...
    static func fetchCachedResponse(uniqueKey: String?, apiObject: APIBase) -> (NSData?, Bool) {
        ...
        let context = TTNetworkCoreDataManager.sharedInstance.createPrivateContext()
        
        let predicate = NSPredicate(format: "%K == %@", Database.APICacheFields.Key , uniqueKey ?? generateUniqueKey(apiObject: apiObject))
        
        let (result, error) = TTNetworkCoreDataManager.sharedInstance.fetchRequest(entity: Database.Entities.APICache, predicate: predicate, sortDescriptors: nil, context: context)
    ...
    static func generateUniqueKey(apiObject : APIBase) -> String {
        ...
        if let parameters = apiObject.requestParameter() {            
            do {
                ...
                paramString = NSString(data:jsonData as Data, encoding:String.Encoding.utf8.rawValue) as String?
    ...
    static func saveResponse(uniqueKey: String?, apiObject: APIBase, cacheData: NSData, ttl: String) {
        ...
        TTNetworkCoreDataManager.sharedInstance.saveContext(context: context)
    }
    ...
    static func clearAllCachedContent() {
        ...
        // delegate objects
        let context = TTNetworkCoreDataManager.sharedInstance.createPrivateContext()
        let persistentStoreCoordinator = TTNetworkCoreDataManager.sharedInstance.persistentStoreCoordinator
